...
module.exports = function(params) {
	...
	}else
	{
		...
		switch(params.deviceType.type){
			...
			case 'saturation':
				light.sendCommands(myCommands.on(zone));
				light.sendCommands(myCommands.saturation(zone, params.state.value, 1));
			break;
			...
			case 'whitetemp':
				light.sendCommands(myCommand.on(zone));
				light.sendCommands(myCommands.whiteTemperature(zone,params.state.value));
			break;
			...
			case 'nightmode':
				if(params.state.value==1){
					light.sendCommands(myCommands.on(zone));
					light.sendCommands(myCommands.nightMode(zone));
				}
			break;
		}
